...
def model (inputs, num_classes, is_training):
    ...
    pool0   = layers.maxpool(conv0_2, 2,  'pool0') 
    ...
    pool1   = layers.maxpool(conv1_2, 4,  'pool1') 
    ...
    drop2   = layers.dropout(pool2, dropout_keep_prob, 'drop2') 
    ...
    drop3   = layers.dropout(pool3, dropout_keep_prob, 'drop3')
    ...
    drop4   = layers.dropout(pool4, dropout_keep_prob, 'drop4')  
    ...
    drop5   = layers.dropout(conv5_2, dropout_keep_prob, 'drop5') 
    ...
    concat6_1 = tf.concat(values=[layers.maxpool(conv0_2, 128, 'down_0_2'), 
                                ...
                                layers.maxpool(conv3_2,   4, 'down_3_2')], axis =2, name ='concat6_1')  
    ...
    drop6 = layers.dropout(conv6_1, dropout_keep_prob, 'drop6')
    ...
    concat7_1 = tf.concat(values=[layers.maxpool(conv0_2, 32, 'down_0_2'), 
                                ...
                                layers.maxpool(conv2_2,  4, 'down_2_2')], axis =2, name ='concat7_1') 
    ...
    drop7 = layers.dropout(conv7_1, dropout_keep_prob, 'drop7') 

    upsample81 = layers.deconv_upsample(drop7, 4, 'upsample8')
    upsample81 = tf.keras.layers.Cropping1D(cropping=((0, 2)))(upsample81)
    concat8_1 = tf.concat(values=[layers.maxpool(conv0_2, 8, 'down_0_2'), layers.maxpool(conv1_2,  4, 'down_1_2')], axis =2, name ='concat8_1')  
    ...
    conv8_1 = layers.conv_btn(concat8, 3, 32, 'conv8_1', is_training = is_training)  
    ...
    conv9_1 = layers.conv_btn(concat9, 3, 32, 'conv9_1', is_training = is_training)  
    ...
    conv10_1 = layers.conv_btn(concat10, 3, 32, 'conv10_1', is_training = is_training) 
...
def segmentation_loss(logits, labels, class_weights = None):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = labels, logits = logits, name = 'segment_cross_entropy')
