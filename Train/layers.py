...
def conv2d(inputs, kernel_size, num_outputs, name,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        weights = tf.compat.v1.get_variable('weights', kernel_shape, tf.float32, tf.compat.v1.glorot_uniform_initializer())
        bias    = tf.compat.v1.get_variable('bias', [num_outputs], tf.float32, tf.constant_initializer(0))
...
def conv_btn(inputs, kernel_size, num_outputs, name,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        # outputs = tf.keras.layers.BatchNormalization(center = True, scale = True)(outputs, training = is_training)
...
def deconv(inputs, kernel_size, num_filters_in, num_outputs, name,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        weights = tf.compat.v1.get_variable('weights', kernel_shape, tf.float32, tf.compat.v1.glorot_uniform_initializer())
        bias    = tf.compat.v1.get_variable('bias', [num_outputs], tf.float32, tf.constant_initializer(0.0))
        conv_trans = tf.nn.conv1d_transpose(inputs, weights, output_shape, stride_shape, padding = padding)
...
def bilinear_upsample_weights2d(factor, num_outputs):
    ...
    init = tf.constant_initializer(value=weights_kernel)
...
def deconv_upsample(inputs, factor, name, padding = 'SAME', activation_fn = None):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        # outputs = tf.keras.layers.Conv1DTranspose(weights,x , strides = stride_shape, padding = padding)(inputs)
...
def fully_connected(inputs, num_outputs, name, activation_fn = tf.nn.relu):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        weights = tf.compat.v1.get_variable('weights', [num_filters_in, num_outputs], tf.float32, tf.compat.v1.initializers())
        bias    = tf.compat.v1.get_variable('bias', [num_outputs], tf.float32, tf.constant_initializer(0.0))
